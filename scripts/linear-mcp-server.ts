...
const tools = [
  {
    ...
    description: "Fetch a Linear issue by identifier, for example ISS-168.",
    inputSchema: {
      type: "object",
      properties: {
        id: { type: "string", description: "Linear issue identifier, for example ISS-168." }
      },
      required: ["id"]
    }
  },
      ...
      case "initialize":
        send({
          ...
          result: {
            protocolVersion,
            capabilities: { tools: {} },
            serverInfo: { name: "indie-suite-linear-mcp", version: "1.0.0" }
          }
        });
        return;
